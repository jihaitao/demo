...
- (void)viewDidLoad
{
    ...
    if ([GlobalMusicPlayRow sharedSingleton].qqMusicHitList) {
        if ([[GlobalMusicPlayRow sharedSingleton].qqMusicHitList isEqualToString:@"newSong"])
        {
            self.navigationItem.title = @"新歌榜";
        }
        else if ([[GlobalMusicPlayRow sharedSingleton].qqMusicHitList isEqualToString:@"allSong"])
        {
...
//    [menu showInView:[UIApplication sharedApplication].keyWindow];
    ...
    NSString *prompInfo = [NSString stringWithFormat:@"是否下载音乐：%@-%@",qqMusicinfo.mSingerName,qqMusicinfo.mSongName];
    UIActionSheet *menu = [[UIActionSheet alloc] initWithTitle:prompInfo delegate:self cancelButtonTitle:@"取消下载" destructiveButtonTitle:@"开始下载" otherButtonTitles:nil, nil];
    [menu showInView:[UIApplication sharedApplication].keyWindow];
    ...
    if (buttonIndex == 0) {
        if (selectRow) {
            ...
            if ([CommonHelper isExistFile:targetPath]) {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"本首歌曲已经下载，是否重新下载?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
                [alert show];
            }
        }
    }
